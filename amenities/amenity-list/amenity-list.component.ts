  ...
  loadAmenities(): void {
    ...
    this.http.get<any>(this.apiUrl).subscribe({
      next: (response: any) => {
        ...
        } else if (response && Array.isArray(response.amenities)) {
          this.amenities = response.amenities;
        } else {
          console.warn('Unexpected amenities response format:', response);
          this.amenities = [];
        }
      ...
      error: (error: any) => {
        ...
        if (error.status === 403) {
          this.snackBar.open('No tienes permisos para ver amenidades', 'Cerrar', { duration: 3000 });
